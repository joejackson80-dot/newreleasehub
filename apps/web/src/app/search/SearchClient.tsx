            ...
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 border-b border-white/5 pb-8">
               {['All', 'Artists', 'Releases', 'Hubs', 'Opportunities'].map(filter => (
                 <button 
                   ...
                   className={`px-5 py-2.5 sm:px-6 rounded-full text-[9px] sm:text-[10px] font-bold uppercase tracking-widest transition-all ${activeFilter === filter ? 'bg-[#A855F7] text-white shadow-[0_0_20px_rgba(168, 85, 247,33)]' : 'text-zinc-500 hover:text-white bg-white/05 border border-white/10'}`}
                 >
                  {filter}
                </button>
              ))}
            ...
            {results.map(item => (
                <div key={item.id} className="bg-[#111] border border-white/5 rounded-3xl p-8 hover:border-white/20 transition-all group relative overflow-hidden">
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center justify-between">
                             ...
                             <div className="text-right">
                               <span className="bg-[#A855F71a] text-[#A855F7] border border-[#A855F733] px-3 py-1 rounded text-[9px] font-bold uppercase tracking-widest">{item.badge}</span>
                        ...
                        <Link href={`/${item.slug}`} className="flex items-center justify-between pt-6 border-t border-white/5 group/link">
                            ...
                            <ArrowRight className="w-4 h-4 text-gray-800 group-hover/link:text-[#A855F7] transition-all group-hover/link:translate-x-1" />
                        </Link>
                      </div>
                  )}
                  ...
                  {item.type === 'hub' && (
                      <div className="space-y-6">
                        <div className="flex items-center justify-between">
                            <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-[#A855F7] shadow-[0_0_20px_rgba(51,102,255,0.3)]">
                              <img src={item.img} className="w-full h-full object-cover" />
                            </div>
                            <div className="text-right">
                        ...
                        <Link href={`/${item.slug}/live`} className="w-full py-4 rounded-2xl bg-[#A855F7] text-white text-center font-bold text-[10px] uppercase tracking-[0.2em] hover:bg-[#2952CC] transition-all flex items-center justify-center gap-2">
                  ...
                  {item.type === 'opportunity' && (
                      <div className="space-y-6 flex flex-col h-full">
                        ...
                        <div className="space-y-2 flex-1">
                            ...
                            <p className="text-xs text-[#A855F7] font-bold uppercase tracking-widest">{item.category}</p>
                        </div>

...
export default function SubscribePage() {
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-white py-32 selection:bg-[#F1F5F9] selection:text-white">
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        
        {/* INSTITUTIONAL LOGO */}
        <div className="flex justify-center mb-20">
           <Link href="/" className="w-12 h-12 rounded-2xl bg-transparent text-black flex items-center justify-center font-bold text-2xl tracking-tighter hover:scale-105 transition-transform shadow-2xl"><img src="/images/nrh-logo.png" alt="NRH Logo" className="w-full h-full object-contain" /></Link>
        </div>
          ...
          {tiers.map((tier, i) => (
            <div 
              ...
              className={`p-10 border rounded-3xl flex flex-col space-y-8 transition-all relative ${
                tier.highlight 
                  ? 'border-[#F1F5F9] bg-[#F1F5F9]/5 shadow-[0_0_40px_rgba(51,102,255,0.1)]' 
                  : 'border-white/5 bg-white/[0.02] hover:bg-white/[0.04]'
              }`}
            >
              {tier.highlight && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#F1F5F9] text-white text-[10px] font-bold px-4 py-1 rounded-full uppercase tracking-widest">
                  Most Popular
                </div>
              )}
              ...
              <div className="space-y-4">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                  tier.highlight ? 'bg-[#F1F5F9] text-white' : 'bg-white/5 text-[#F1F5F9]'
                }`}>
              ...
              <div className="flex-grow space-y-4 pt-8 border-t border-white/5">
                {tier.features.map((feature, j) => (
                  <div key={j} className="flex items-start space-x-3 text-sm">
                    <Check className="w-4 h-4 text-[#F1F5F9] mt-0.5" />
              ...
              <button 
                ...
                className={`w-full py-5 rounded-2xl font-bold uppercase tracking-widest text-center transition-all flex items-center justify-center space-x-2 ${
                  tier.highlight 
                    ? 'bg-[#F1F5F9] text-white hover:bg-[#F1F5F9]/90 shadow-lg shadow-[#F1F5F9]/20' 
        ...
        <div className="mt-32 p-10 border border-white/5 rounded-3xl bg-white/[0.01] flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center space-x-6">
            <div className="w-16 h-16 rounded-2xl bg-[#F1F5F9]/10 flex items-center justify-center text-[#F1F5F9]">
          ...
          <Link 
            ...
            className="flex items-center space-x-2 text-[#F1F5F9] font-bold text-xs uppercase tracking-widest hover:underline"
          >

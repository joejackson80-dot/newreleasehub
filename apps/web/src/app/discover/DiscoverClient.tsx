      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 space-y-20">
           ...
           <div className="flex items-center gap-4 w-full md:w-auto">
              <div className="relative flex-1 md:w-80">
                 ...
                 <input 
                   ...
                   className="w-full bg-[#111] border border-white/10 rounded-2xl pl-12 pr-4 py-4 text-xs font-bold uppercase tracking-widest text-white focus:outline-none focus:border-[#A855F7] transition-all" 
                 />
              </div>
           ...
           <div className="flex items-center gap-2 overflow-x-auto no-scrollbar px-2">
              {genres.map((genre) => (
                 <button
                   ...
                   className={`text-[10px] font-bold uppercase tracking-widest px-6 py-3 rounded-xl transition-all whitespace-nowrap ${activeGenre === genre ? 'bg-[#A855F7] text-white' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}
                 >
                   {genre}
                 </button>
              ))}
        ...
        <section className="space-y-10">
           <div className="flex items-center justify-between">
              ...
              <Link href="/network/charts" className="text-[10px] font-bold text-[#A855F7] uppercase tracking-widest hover:text-white transition-colors">View All Charts</Link>
           </div>
           {filteredArtists.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8">
                 {filteredArtists.map((artist: any) => (
                    <Link key={artist.id} href={`/${artist.slug}`} className="group space-y-4">
                       <div className="aspect-square rounded-3xl overflow-hidden bg-zinc-900 relative border border-white/5 group-hover:border-[#A855F7]/50 transition-all">
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                             <div className="w-12 h-12 rounded-full bg-[#A855F7] flex items-center justify-center text-white">
                     ...
                     <div key={release.id} className="bg-[#111] border border-white/5 rounded-3xl p-6 flex items-center gap-6 group hover:bg-white/[0.02] transition-all">
                        ...
                        >
                           ...
                           <div className={`absolute inset-0 bg-black/40 flex items-center justify-center transition-opacity ${isThisPlaying ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                              {isThisPlaying ? <Pause className="w-6 h-6 text-[#A855F7] fill-current" /> : <Play className="w-6 h-6 text-white fill-current ml-1" />}
                           </div>
                        </div>
                        <div className="flex-1 min-w-0 space-y-2">
                           <h4 className="font-bold text-lg truncate hover:text-[#A855F7] transition-colors cursor-pointer" onClick={(e) => handlePlay(e, release)}>{release.title}</h4>
                           <Link href={`/${release.Organization?.slug || ''}`} className="text-[10px] font-bold text-gray-500 hover:text-[#A855F7] uppercase tracking-widest transition-colors block truncate">
                              {release.Organization?.name || 'Unknown Artist'}
                           </Link>
                           <div className="flex items-center justify-between pt-4">
                              <span className="text-[9px] font-bold text-[#A855F7] uppercase tracking-widest">{release.type || 'Single'}</span>

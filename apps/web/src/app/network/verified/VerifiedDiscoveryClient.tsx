        ...
        <section className="space-y-8">
           <div className="flex flex-col lg:flex-row gap-6">
              <div className="relative flex-1 group">
                 <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-600 group-focus-within:text-[#F1F5F9] transition-colors" />
                 <input 
                   type="text" 
                   placeholder="SEARCH THE NETWORK MATRIX..."
                   value={query}
                   onChange={(e) => setQuery(e.target.value)}
                   className="w-full bg-[#0A0A0A] border border-white/5 rounded-3xl py-6 pl-16 pr-8 text-xs font-bold uppercase tracking-[0.2em] focus:outline-none focus:border-[#F1F5F9]/30 transition-all shadow-2xl"
                 />
              </div>
              
              <button 
                onClick={() => setShowFilters(!showFilters)}
                className={`px-8 py-6 rounded-3xl border transition-all flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest ${showFilters ? 'bg-[#F1F5F9] border-[#F1F5F9] text-white' : 'bg-[#0A0A0A] border-white/5 text-gray-400 hover:border-white/20'}`}
              >
                        ...
                        <div className="grid grid-cols-2 gap-3">
                           {SORT_OPTIONS.map(opt => (
                             <button 
                               ...
                               className={`px-4 py-4 rounded-xl text-[9px] font-bold uppercase tracking-widest text-left transition-all border ${sortBy === opt.id ? 'bg-[#F1F5F9]/10 border-[#F1F5F9]/40 text-[#F1F5F9]' : 'bg-white/5 border-transparent text-gray-500 hover:border-white/10'}`}
                             >
                               {opt.label}
                             </button>
                           ))}
                     ...
                     <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                           ...
                           <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                              <div className="h-full bg-[#F1F5F9]" style={{ width: '40%' }}></div>
                           </div>
                        </div>
                     </div>
                  </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/5 pb-6 gap-6">
              ...
              <div className="flex items-center gap-6">
                 ...
                 <div className="hidden sm:flex items-center gap-4 text-[9px] font-bold text-gray-600 uppercase tracking-widest">
                    ...
                    <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#F1F5F9]"></div> VERIFIED EQUITY</div>
                 </div>
              </div>
            </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {artists.map((artist, i) => (
                  <motion.div 
                    ...
                    className="group bg-[#0A0A0A] border border-white/5 rounded-[3rem] p-10 space-y-10 hover:border-[#F1F5F9]/30 transition-all shadow-2xl relative overflow-hidden"
                  >
                     <div className="flex justify-between items-start relative z-10">
                        <div className="relative">
                           <div className="w-20 h-20 rounded-[2rem] bg-zinc-900 border border-white/10 overflow-hidden group-hover:border-[#F1F5F9]/40 transition-colors">
                           ...
                           {artist.isVerified && (
                             <div className="absolute -bottom-2 -right-2 bg-[#F1F5F9] text-white p-2 rounded-xl shadow-xl">
                     ...
                     <div className="space-y-3 relative z-10">
                        ...
                        <h4 className="text-3xl font-black italic uppercase tracking-tighter text-white group-hover:text-[#F1F5F9] transition-colors">{artist.name}</h4>
                     ...
                     {/* BG GLOW */}
                     <div className="absolute top-0 right-0 w-32 h-32 bg-[#F1F5F9]/5 blur-3xl rounded-full group-hover:bg-[#F1F5F9]/10 transition-all"></div>
                  </motion.div>
                ))}
             </div>
           ) : (
                              ...
                              <td className="p-8">
                                 <div className="flex items-center gap-3">
                                    <div className="w-16 h-1.5 bg-white/5 rounded-full overflow-hidden">
                                       <div className="h-full bg-[#F1F5F9]" style={{ width: `${artist.verifiedScore}%` }}></div>
                                    </div>
                                    <span className="text-sm font-black italic text-white">{artist.verifiedScore}</span>
                                 </div>
                              </td>
                              <td className="p-8 text-right">
                                 <Link href={`/fan/${artist.slug}`} className="inline-flex items-center gap-2 text-[10px] font-black text-white uppercase tracking-widest hover:text-[#F1F5F9] transition-colors">

    ...
    <div className="min-h-screen bg-[#020202] text-white flex flex-col items-center justify-center px-6 py-20">
          ...
          {[1, 2, 3].map(s => (
            <React.Fragment key={s}>
              <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
                step > s ? 'bg-[#F1F5F9] border-[#F1F5F9]' :
                step === s ? 'border-[#F1F5F9] bg-[#F1F5F9]/10' : 'border-white/10 bg-white/5'
              }`}>
                {step > s
              ...
              {s < 3 && <div className={`flex-1 h-px transition-all ${step > s ? 'bg-[#F1F5F9]' : 'bg-white/10'}`} />}
            </React.Fragment>
          ))}
        </div>
      </div>
      ...
      {step === 1 && (
        <div className="w-full max-w-2xl space-y-10">
          <div className="text-center space-y-4">
            <div className="w-14 h-14 rounded-2xl bg-[#F1F5F9]/10 border border-[#F1F5F9]/20 flex items-center justify-center mx-auto">
              <Music className="w-6 h-6 text-[#F1F5F9]" />
            </div>
              ...
              <button key={g} onClick={() => toggleGenre(g)}
                className={`px-5 py-2.5 rounded-full border text-sm font-bold transition-all ${
                  selectedGenres.includes(g)
                    ? 'bg-[#F1F5F9] border-[#F1F5F9] text-white shadow-lg shadow-[#F1F5F9]/20'
      ...
      {step === 2 && (
        <div className="w-full max-w-2xl space-y-10">
          <div className="text-center space-y-4">
            <div className="w-14 h-14 rounded-2xl bg-[#F1F5F9]/10 border border-[#F1F5F9]/20 flex items-center justify-center mx-auto">
              <Users className="w-6 h-6 text-[#F1F5F9]" />
            </div>
              ...
              return (
                <button key={artist.id} onClick={() => toggleFollow(artist.id)}
                  className={`relative group rounded-2xl overflow-hidden border transition-all text-left ${
                    isFollowed ? 'border-[#F1F5F9] ring-1 ring-[#F1F5F9]' : 'border-white/5 hover:border-white/20'
                  }`}>
                  <div className="aspect-square relative">
                    <img src={artist.photo} alt={artist.name} className="w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent" />
                    {isFollowed && (
                      <div className="absolute top-3 right-3 w-7 h-7 bg-[#F1F5F9] rounded-full flex items-center justify-center shadow-lg">
                        <Check className="w-4 h-4 text-white" />
                      </div>
                    )}
                  ...
                  <div className={`py-2.5 text-center text-[10px] font-bold uppercase tracking-wider transition-all ${
                    isFollowed ? 'bg-[#F1F5F9] text-white' : 'bg-white/5 text-gray-400 group-hover:text-white'
                  }`}>
      ...
      {step === 3 && (
        <div className="w-full max-w-lg text-center space-y-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/fan/me"
              className="px-10 py-4 rounded-xl bg-white text-black font-bold text-xs uppercase tracking-widest hover:bg-[#F1F5F9] hover:text-white transition-all">
              Go to My Feed
            </Link>
            <Link href="/discover"

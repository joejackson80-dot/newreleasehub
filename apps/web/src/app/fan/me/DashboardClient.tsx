          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="text-right hidden md:block px-6 border-r border-white/10">
               ...
               <div className="flex items-center gap-3">
                  <div className="w-24 h-2 bg-white/10 rounded-full overflow-hidden">
                     <div className="h-full bg-gradient-to-r from-purple-500 to-[#A855F7]" style={{ width: `${(user.fanXP / (user.fanLevel * 500)) * 100}%` }}></div>
                  </div>
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                {unreadNotifs > 0 && (
                  <span className="absolute -top-1 -right-1 w-5 h-5 bg-[#A855F7] border-4 border-black rounded-full text-[8px] font-bold flex items-center justify-center">
                    {unreadNotifs}
                  </span>
                )}
              </div>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-12 mt-12">
             ...
             {TABS.map(t => (
               <button key={t} onClick={() => setActiveTab(t)}
                 ...
                 {t === 'Notifications' && unreadNotifs > 0 && (
                   <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#A855F7] rounded-full text-[8px] font-bold flex items-center justify-center text-white">
                     {unreadNotifs}
                   </span>
                 )}
               </button>
          ...
          <div className="space-y-6">
            {isLoadingFeed ? (
               ...
               <div className="bg-[#111] border border-white/5 rounded-3xl p-12 text-center flex flex-col items-center justify-center space-y-4">
                  ...
                  <Link href="/discover" className="text-[#A855F7] font-bold text-xs">Explore Artists</Link>
               </div>
            ) : (
               feed.map(post => (
                   ...
                   <div className="flex items-center gap-3 p-5 pb-0">
                     ...
                     <div className="flex-1">
                       <div className="flex items-center gap-2">
                         <Link href={`/${post.Organization?.slug || post.artistSlug || ''}`} className="font-bold text-sm text-white hover:text-[#A855F7] transition-colors">
                           {post.Organization?.name || post.artistName}
                         </Link>
                         {(post.Organization?.isVerified || post.isVerified) && <Check className="w-3.5 h-3.5 text-[#A855F7]" />}
        ...
        {activeTab === 'Library' && (
          <div className="space-y-4">
            {isLoadingLibrary ? (
               <div className="text-center py-20 text-[#A855F7] text-[10px] font-bold uppercase tracking-widest animate-pulse">Loading Library...</div>
               ...
               <div className="bg-[#111] border border-white/5 rounded-3xl p-12 text-center flex flex-col items-center justify-center space-y-4 hover:border-white/10 transition-colors">
                  ...
                  <Link href="/discover" className="mt-4 px-8 py-3 bg-[#A855F7] text-white text-[10px] font-bold uppercase tracking-widest rounded-xl hover:bg-[#00B8E0] transition-colors shadow-lg">
                     Go to Discover
                  </Link>
               </div>
            ) : (
                     ...
                     <div key={track.id} className="flex items-center gap-4 p-4 bg-[#111] border border-white/5 rounded-2xl hover:bg-white/[0.02] hover:border-white/10 transition-all group">
                        <div className="w-12 h-12 rounded-xl bg-zinc-900 shrink-0 overflow-hidden relative cursor-pointer shadow-lg" onClick={() => handlePlayTrack(track)}>
                           ...
                           <div className={`absolute inset-0 bg-black/40 flex items-center justify-center transition-opacity ${isThisPlaying ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                              {isThisPlaying ? <Pause className="w-4 h-4 text-[#A855F7] fill-current" /> : <Play className="w-4 h-4 text-white fill-current ml-0.5" />}
                           </div>
                        </div>
                        <div className="flex-1 min-w-0">
                           <p className="font-bold text-white truncate text-sm hover:text-[#A855F7] cursor-pointer transition-colors" onClick={() => handlePlayTrack(track)}>{track.title}</p>
                        ...
                        <div className="hidden sm:block text-[10px] font-bold text-[#A855F7] uppercase tracking-widest text-right bg-[#A855F7]/10 px-3 py-1.5 rounded-lg">
                           Saved
                        </div>
                     </div>
                  )})}
                  ...
                  <div className="space-y-6">
                     ...
                     <div className="space-y-4 pt-4 border-t border-white/5">
                        ...
                        <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
                           ...
                           <span className="text-[#A855F7]">Top 15%</span>
                        </div>
                     </div>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              {subscriptions.length === 0 ? (
                <div className="col-span-full text-center py-20 bg-white/5 border border-dashed border-white/10 rounded-3xl">
                   ...
                   <Link href="/discover" className="text-[#A855F7] hover:text-white transition-colors font-bold text-xs">Explore Artists</Link>
                </div>
        ...
        {activeTab === 'Stats' && (
          <div className="space-y-8 animate-in fade-in duration-700">
            {isLoadingStats ? (
              <div className="text-center py-20 text-[#A855F7] text-[10px] font-bold uppercase tracking-widest animate-pulse">Synchronizing Terminal Data...</div>
            ) : (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    { label: 'Total Streams', value: fanStats?.totalStreamsAllTime || 0, icon: Play, color: 'text-[#A855F7]' },
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                   <div className="bg-[#111] border border-white/5 rounded-[2.5rem] p-10 space-y-8 relative overflow-hidden group">
                      <div className="absolute top-0 right-0 p-8 opacity-[0.05] group-hover:scale-110 transition-transform">
                         <TrendingUp className="w-20 h-20 text-[#A855F7]" />
                      </div>
                      <div className="space-y-1">
                      ...
                      <div className="grid grid-cols-2 gap-6">
                         ...
                         <div className="flex items-center justify-center">
                            <div className="relative w-32 h-32">
                               ...
                               <div className="absolute inset-0 border-[10px] border-[#A855F7] rounded-full border-t-transparent border-l-transparent rotate-45" />
                               <div className="absolute inset-0 flex flex-col items-center justify-center">
                                  <Globe className="w-6 h-6 text-[#A855F7] mb-1" />
                ...
                <div className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-10 flex flex-col md:flex-row items-center gap-10">
                   ...
                   <Link href="/governance" className="px-8 py-4 rounded-xl bg-white text-black font-black text-[10px] uppercase tracking-[0.2em] hover:bg-[#A855F7] hover:text-white transition-all shadow-xl">
                      Manage Equity
                   </Link>
                </div>
              </>
            ...
            {subscriptions.length > 0 ? (
              subscriptions.map((sub, i) => (
                <div key={i} className="flex items-center gap-4 p-5 bg-[#111] border border-white/5 rounded-2xl hover:border-white/10 transition-all group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <Link href={`/${sub.Organization.slug}`} className="font-bold text-white hover:text-[#A855F7] transition-colors truncate">{sub.Organization.name}</Link>
                      {sub.Organization.isVerified && <Check className="w-3.5 h-3.5 text-[#A855F7]" />}
                    </div>
                  ...
                  <Link href={`/${sub.Organization.slug}`}
                    className="px-6 py-3 rounded-xl bg-white text-black text-[10px] font-bold uppercase tracking-widest hover:bg-[#A855F7] hover:text-white transition-all shadow-xl">
                    Enter Hub
                  </Link>
                </div>
              ))
        ...
        {activeTab === 'Notifications' && (
          <div className="space-y-3">
            {MOCK_NOTIFICATIONS.map(n => (
              <div key={n.id}
                className={`p-5 rounded-2xl border transition-all ${n.unread ? 'bg-[#A855F7]/5 border-[#A855F7]/20' : 'bg-[#111] border-white/5'}`}>
                <div className="flex items-start gap-3">
                  {n.unread && <div className="w-2 h-2 rounded-full bg-[#A855F7] mt-1.5 shrink-0" />}
                  <div className="flex-1">
                    ...
                    {vaultReleases.map(release => (
                       <div key={release.id} className="group space-y-4">
                          <div className="aspect-square rounded-[2rem] overflow-hidden border border-white/5 relative shadow-2xl">
                             ...
                             <div className="absolute top-4 right-4 px-3 py-1 rounded-lg bg-black/60 backdrop-blur-md border border-white/10 text-[8px] font-bold text-[#A855F7] uppercase tracking-widest">Vault Exclusive</div>
                          </div>
                          <div className="space-y-1">

...
export default function FraudDashboardClient({ flaggedIncidents, topStreamers, excludedStreams, ipClusters }: any) {
  ...
  const stats = [
    ...
    { label: 'Detected Rings', value: Math.floor(ipClusters.length / 3), icon: Zap, color: 'text-[#F1F5F9]' },
  ];
      ...
      <div className="max-w-7xl mx-auto space-y-20">
                              ...
                              <tr key={streamer.deviceId} className="group hover:bg-white/[0.01] transition-colors">
                                 <td className="py-8">
                                    <div className="flex items-center gap-3">
                                       <Fingerprint className="w-4 h-4 text-gray-600 group-hover:text-[#F1F5F9] transition-colors" />
        ...
        <section className="bg-white/[0.02] border border-white/5 rounded-[3rem] p-12 lg:p-20 flex flex-col md:flex-row items-center gap-12 text-center md:text-left">
           ...
           <div className="flex-1 flex justify-center md:justify-end">
              <button className="text-[10px] font-bold text-[#F1F5F9] uppercase tracking-widest hover:underline whitespace-nowrap">View Network Whitepaper →</button>
           </div>
        </section>

      ...
      <div className="max-w-7xl mx-auto space-y-24 sm:space-y-32">
        ...
        <section className="text-center space-y-8">
           <div className="space-y-4">
              <h1 className="text-[clamp(2.5rem,10vw,8rem)] font-bold tracking-tighter italic uppercase leading-[0.8] mb-6">Simple pricing.<br /><span className="text-[#F1F5F9]">Serious payouts.</span></h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          {ARTIST_PLANS.map((tier) => (
            <div key={tier.name} className={`bg-[#111] border ${tier.color} rounded-[2.5rem] p-8 sm:p-10 flex flex-col space-y-10 relative overflow-hidden group hover:bg-white/[0.02] transition-all duration-500 shadow-2xl`}>
              {tier.popular && (
                <div className="absolute top-0 right-0 bg-[#F1F5F9] text-white text-[9px] font-bold uppercase tracking-widest px-6 py-2 rounded-bl-2xl">
                  Most Popular
                </div>
              )}
              <div className="space-y-4">
              ...
              <ul className="flex-1 space-y-4">
                {tier.features.map((feat, i) => (
                  <li key={i} className="flex items-start space-x-3 text-xs font-medium">
                    <Check className="w-4 h-4 text-[#F1F5F9] shrink-0 mt-0.5" />
                    <span className="text-gray-300">{feat}</span>
                  </li>
                ))}
              </ul>
              <Link href={tier.href} className={`block w-full py-4 rounded-xl font-bold text-[10px] uppercase tracking-widest text-center transition-all ${tier.popular ? 'bg-[#F1F5F9] text-white hover:bg-[#00B8E0]' : 'bg-white/5 border border-white/10 text-white hover:bg-white hover:text-black'}`}>
                {tier.buttonText}
              </Link>
            </div>
          ))}
        ...
        <section className="bg-white/5 border border-white/10 rounded-[2rem] sm:rounded-[3rem] p-8 sm:p-12 text-center space-y-8 backdrop-blur-xl">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               ...
               <div className="space-y-2">
                  <p className="text-[#F1F5F9] text-[10px] font-bold uppercase tracking-widest">Pro</p>
              ...
              {FAQS.map((faq, i) => (
                <div key={i} className="space-y-4">
                   <h4 className="text-sm font-bold uppercase tracking-widest flex items-center gap-3">
                      <HelpCircle className="w-4 h-4 text-[#F1F5F9]" />
                      {faq.q}
                   </h4>

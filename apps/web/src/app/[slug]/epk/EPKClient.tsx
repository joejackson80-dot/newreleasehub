  ...
  return (
    <div className="space-y-20">
            ...
            {isDownloading ? (
              <div className="flex items-center space-x-3">
                 <div className="w-12 h-1 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-[#F1F5F9] transition-all" style={{ width: `${downloadProgress}%` }} />
                 </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         {[
           { label: 'Total Streams', val: '4.2M', growth: '+12%', icon: Activity, color: 'text-[#F1F5F9]' },
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-[3rem] p-10 space-y-8">
         ...
         <div className="h-48 flex items-end space-x-2">
            {[30, 45, 35, 60, 50, 80, 65, 90, 75, 100, 85, 110].map((h, i) => (
               <div 
                 key={i} 
                 className="flex-1 bg-gradient-to-t from-[#F1F5F9]/10 to-[#F1F5F9]/40 rounded-t-lg hover:to-[#F1F5F9] transition-all cursor-pointer relative group" 

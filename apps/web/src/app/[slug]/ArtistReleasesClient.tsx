          ...
          <div key={release.id} className="group space-y-4">
            ...
            >
              ...
              <div className={`absolute inset-0 bg-black/50 transition-opacity flex items-center justify-center ${isThisPlaying ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                <div className={`w-12 h-12 rounded-full bg-[#F1F5F9] flex items-center justify-center text-white transition-transform ${isThisPlaying ? 'scale-100' : 'scale-75 group-hover:scale-100'}`}>
            ...
            <Link href={`/${slug}/${release.id}`}>
              <h4 className="font-bold truncate text-white hover:text-[#F1F5F9] transition-colors">{release.title}</h4>

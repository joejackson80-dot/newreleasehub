...
import { Play, Pause, ChevronLeft, Share2, Heart, Download, Info, Disc, Calendar, ShieldCheck } from 'lucide-react';
import Link from 'next/link';
import { useAudio } from '@/context/AudioContext';

export default function ReleaseDetailClient({ release }: { release: any }) {
  const { artistName, slug } = release;
  ...
  const handlePlay = () => {
    ...
    playTrack({
      ...
      imageUrl: release.coverArtUrl || '/images/default-cover.png'
    });
  };

  return (
    <div className="min-h-screen bg-[#020202] text-white selection:bg-[#A855F7] selection:text-white pt-32 pb-32">
         ...
         <section className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            ...
            <div className="lg:col-span-5 space-y-10">
               <div className="relative aspect-square rounded-[3rem] overflow-hidden border border-white/10 shadow-2xl group">
                  <img src={release.coverArtUrl || '/images/default-cover.png'} alt={release.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />
                  <div className="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-colors" />
                  <button 
                    onClick={handlePlay}
                    className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                  >
                     <div className="w-24 h-24 rounded-full bg-[#A855F7] flex items-center justify-center text-white scale-75 group-hover:scale-100 transition-transform shadow-2xl">
            ...
            <div className="lg:col-span-7 space-y-12">
               <div className="space-y-6">
                  <Link href={`/${slug}`} className="inline-flex items-center space-x-3 text-zinc-500 hover:text-[#A855F7] transition-colors group">
               ...
               <div className="flex flex-wrap gap-10 items-center border-y border-white/5 py-10">
                  <div className="flex items-center gap-4">
                     <div className="w-10 h-10 rounded-full bg-zinc-900 flex items-center justify-center text-[#A855F7]">
               ...
               <div className="pt-6">
                  <button 
                    onClick={handlePlay}
                    className="w-full sm:w-auto px-16 py-6 rounded-full bg-white text-black font-black text-xs uppercase tracking-[0.2em] hover:bg-[#A855F7] hover:text-white transition-all shadow-2xl flex items-center justify-center gap-4 group"
                  >
         ...
         <section className="grid grid-cols-1 lg:grid-cols-2 gap-20 pt-24 border-t border-white/5">
            ...
            <div className="space-y-12">
               ...
               <div className="space-y-2">
                  {(release.tracks || [{ id: 't1', title: release.title, duration: '3:45' }]).map((track: any, i: number) => (
                    <div key={track.id} className="group flex items-center justify-between p-6 rounded-3xl hover:bg-white/5 transition-all border border-transparent hover:border-white/5">
                       <div className="flex items-center gap-6">
                          ...
                          <div>
                             <h5 className="font-bold uppercase tracking-tight group-hover:text-[#A855F7] transition-colors">{track.title}</h5>
            ...
            <div className="space-y-12">
               ...
               <div className="bg-[#111] border border-white/5 rounded-[3rem] p-10 space-y-10">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                     {(release.credits || []).map((credit: any, i: number) => (

...
const ACCENT_SWATCHES = ['#A855F7', '#E63946', '#2EC4B6', '#FF9F1C', '#8338EC', '#06D6A0', '#FFBE0B', '#FB5607'];
  ...
  return (
    <div className="flex h-[calc(100vh-0px)] overflow-hidden">
      ...
      <div className="w-[40%] border-r border-white/5 overflow-y-auto p-8 space-y-6">
                ...
                {FONT_OPTIONS.map(f => (
                  <button key={f.name} onClick={() => setSelectedFont(f.name)}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {selectedFont === f.name && <div className="w-2 h-2 rounded-full bg-[#A855F7]" />}
                    </div>
                ...
                {HERO_STYLES.map(s => (
                  <button key={s} onClick={() => setHeroStyle(s)}
                    className={`p-4 rounded-xl border text-left transition-all ${
                      heroStyle === s ? 'bg-[#A855F7]/10 border-[#A855F7]/40 text-white' : 'border-white/10 text-gray-400 hover:text-white bg-[#111]'
                    }`}>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button 
                  ...
                  className="text-[9px] font-bold text-[#A855F7] uppercase tracking-widest hover:text-white transition-colors flex items-center gap-1.5 disabled:opacity-50"
                >
            ...
            <div className="space-y-4">
               ...
               <div className="space-y-2">
                  {[
                    ...
                    <div key={m.id} className="p-4 bg-white/5 border border-white/5 rounded-xl flex items-center justify-between group hover:border-[#A855F7]/30 transition-all">
                       ...
                       <input type="checkbox" className="w-4 h-4 rounded border-white/10 bg-transparent text-[#A855F7]" defaultChecked />
                    </div>
                  ))}
               </div>
            </div>
        ...
        {tab === 'broadcast' && (
          <div className="space-y-8">
            <div className="p-6 bg-[#A855F7]/5 border border-[#A855F7]/10 rounded-2xl space-y-4">
              ...
              <textarea 
                ...
                className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#A855F7]/30 resize-none"
                rows={3}
              />
              <button 
                onClick={() => { alert('Broadcast Sent to Network!'); setBroadcastMsg(''); }}
                className="w-full py-3 bg-[#A855F7] text-white font-bold text-[10px] uppercase tracking-widest rounded-xl hover:scale-[1.02] transition-all shadow-lg"
              >
                Send Announcement
              </button>
            </div>
        ...
        <button onClick={handleSave}
          className={`w-full py-4 rounded-xl font-bold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
            saved ? 'bg-green-500 text-white' : 'bg-[#A855F7] text-white hover:bg-[#00B8E0]'
          }`}>
        ...
        <div className={`w-full h-full overflow-y-auto no-scrollbar pb-32 ${getFontClass()}`}>
           ...
           <div className="px-12 mt-16 space-y-6">
              <div className="flex items-center justify-between">
                 ...
                 <span className="text-[8px] font-black text-[#A855F7] uppercase tracking-widest">3 Verified Assets</span>
              </div>
              <div className="grid grid-cols-2 gap-4">
                 {[1, 2].map(i => (
                    <div key={i} className="p-6 rounded-[2rem] bg-white/5 border border-white/10 space-y-4 hover:border-[#A855F7]/40 transition-all cursor-pointer">
                       <div className="flex items-center justify-between">
                          <div className="w-10 h-10 rounded-xl bg-zinc-900 border border-white/10 flex items-center justify-center">
                             <Music className="w-5 h-5 text-gray-500" />
                          </div>
                          <ShieldCheck className="w-4 h-4 text-[#A855F7] opacity-50" />
                       </div>

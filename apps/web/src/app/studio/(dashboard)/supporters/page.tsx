  ...
  return (
    <div className="p-8 md:p-12 space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button className="bg-[#A855F7] text-white px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-[#00B8E0] transition-all flex items-center gap-2 w-full md:w-auto justify-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {org.SupporterTiers.map(tier => (
          <div key={tier.id} className="bg-[#111] border border-white/5 rounded-2xl p-6 relative group hover:border-[#A855F7]/50 transition-colors flex flex-col h-full">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h4 className="font-bold text-lg text-white">{tier.name}</h4>
                <p className="text-[#A855F7] font-bold text-2xl mt-1">${(tier.priceCents / 100).toFixed(2)}<span className="text-xs text-gray-500 font-medium">/mo</span></p>
              </div>

...
export default function AnalyticsClient({ artist }: { artist: any }) {
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-white p-6 sm:p-8 md:p-12 space-y-12 selection:bg-[#F1F5F9] selection:text-black">
         ...
         <div className="flex items-center gap-4">
            ...
            <div className="bg-[#0A0A0A] border border-white/5 p-4 rounded-2xl flex items-center gap-6">
               <div className="text-right">
                  ...
                  <p className="text-xl font-black italic text-[#F1F5F9]">TOP 2%</p>
               </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
         {[
           ...
           { label: 'Verified Plays', value: stats?.verifiedStreams?.toLocaleString() || '0', sub: 'Protocol Authorized', icon: ShieldAlert, color: 'text-[#F1F5F9]' },
           ...
           <div key={i} className="bg-[#0A0A0A] border border-white/10 p-5 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] space-y-4 relative group hover:border-[#F1F5F966] transition-all cursor-default overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-2 bg-[#0A0A0A] border border-white/5 rounded-[3.5rem] p-10 space-y-8 relative overflow-hidden">
            <div className="flex items-center justify-between relative z-10">
               ...
               <div className="flex gap-2">
                  {['7D', '14D', '30D', '90D'].map(t => (
                    <button key={t} className={`px-4 py-2 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all ${t === '14D' ? 'bg-[#F1F5F9] text-black shadow-lg' : 'bg-white/5 text-zinc-500 hover:text-white'}`}>
                       {t}
                    </button>
                  ))}
               </div>
            ...
            <div className="h-64 flex items-end gap-2 relative z-10">
               {stats?.dailyStreams?.map((s: any, i: number) => (
                 <div key={i} className="flex-1 group relative">
                    <div 
                      className="w-full bg-gradient-to-t from-[#F1F5F966] to-[#F1F5F9] rounded-t-lg transition-all group-hover:scale-y-105" 
            ...
            {/* DECOR */}
            <div className="absolute -bottom-20 -right-20 w-64 h-64 bg-[#F1F5F9]/5 blur-[100px] rounded-full pointer-events-none" />
         </div>
               ...
               {stats?.countryDistribution && Object.entries(stats.countryDistribution).slice(0, 5).map(([country, count]: [string, any], i) => (
                 <div key={country} className="space-y-2 group">
                    <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
                       <span className="text-white flex items-center gap-2 group-hover:text-[#F1F5F9] transition-colors">
                    ...
                    <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                       <div 
                         className="h-full bg-[#F1F5F9] transition-all duration-1000 shadow-[0_0_10px_rgba(241,245,249,16)]" 
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
         ...
         <div className="bg-[#0A0A0A] border border-white/5 rounded-[2.5rem] sm:rounded-[3.5rem] p-8 sm:p-12 flex flex-col sm:flex-row gap-8 sm:gap-12 items-center">
            <div className="relative w-32 h-32 sm:w-48 sm:h-48 shrink-0">
               <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                  ...
                  <circle 
                    cx="50" cy="50" r="45" fill="none" stroke="#F1F5F9" strokeWidth="8" 
         ...
         <div className="bg-gradient-to-br from-zinc-900 to-[#020202] border border-white/10 rounded-[3.5rem] p-12 space-y-8 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-12 opacity-5 group-hover:opacity-10 transition-opacity">
               <TrendingUp className="w-48 h-48 text-[#F1F5F9]" />
            </div>
            ...
            <div className="space-y-6 relative z-10">
               ...
               <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="p-6 bg-white/5 border border-white/10 rounded-3xl space-y-2">
                     ...
                     <p className="text-xl font-black italic text-[#F1F5F9]">$3,160</p>
                  </div>
               </div>
            </div>
         </div>
      ...
      <footer className="pt-20 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-8">
         ...
         <button className="px-12 py-5 bg-[#F1F5F9] text-black font-black uppercase tracking-widest text-[11px] rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-[0_0_50px_rgba(241,245,249,0.2)]">

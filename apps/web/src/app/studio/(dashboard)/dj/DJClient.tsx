      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
         ...
         <div className="flex items-center space-x-4">
            ...
            <button 
               ...
               disabled={isLoading}
               className={`px-10 py-4 rounded-xl font-bold text-xs uppercase tracking-widest transition-all shadow-xl disabled:opacity-50 ${isLive ? 'bg-red-500 hover:bg-red-600 shadow-red-500/20' : 'bg-[#F1F5F9] hover:bg-[#00B8E0] shadow-blue-500/20'}`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                  ...
                  <div className="absolute inset-0">
                     ...
                     <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center space-y-4">
                           <Activity className="w-12 h-12 text-[#F1F5F9] animate-pulse mx-auto" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="bg-[#111] border border-white/5 rounded-3xl p-8 space-y-4">
                  ...
                  <div className="flex items-center justify-between bg-black/50 border border-white/5 p-4 rounded-2xl">
                     <code className="text-xs text-[#F1F5F9] truncate mr-4">{streamConfig?.rtmpUrl || 'Loading...'}</code>
                     <button 
               ...
               <div className="bg-[#111] border border-white/5 rounded-3xl p-8 space-y-4">
                  ...
                  <div className="flex items-center justify-between bg-black/50 border border-white/5 p-4 rounded-2xl">
                     <code className="text-xs text-[#F1F5F9] truncate mr-4">
         ...
         <div className="space-y-10">
            <div className="bg-[#111] border border-white/5 rounded-[2.5rem] flex flex-col h-[600px] overflow-hidden shadow-2xl">
               <div className="p-8 border-b border-white/5 flex items-center justify-between bg-[#151515]">
                  <div className="flex items-center space-x-3">
                     <MessageCircle className="w-4 h-4 text-[#F1F5F9]" />
                     <h3 className="text-xs font-bold uppercase tracking-widest text-white">Live Network Chat</h3>
                  </div>
                  <span className="bg-[#F1F5F9]/10 text-[#F1F5F9] px-3 py-1 rounded-full text-[9px] font-mono font-bold">
                     ...
                     messages.map((msg, i) => (
                        <div key={i} className="space-y-1 group">
                           <div className="flex items-center justify-between">
                              <span className="text-[10px] font-bold text-[#F1F5F9] uppercase tracking-widest italic">{msg.user}</span>
               ...
               <div className="p-8 bg-[#0a0a0a] border-t border-white/5">
                  <div className="relative">
                     <input 
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-xs font-bold text-white focus:outline-none focus:border-[#F1F5F9] transition-all disabled:opacity-50"
                     />
                     <button 
                        onClick={handleSendMessage}
                        disabled={!isLive || isSending}
                        className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-700 hover:text-[#F1F5F9] transition-colors disabled:opacity-50"
                     >
            ...
            <div className="bg-white/[0.01] border border-white/5 rounded-3xl p-8 space-y-6">
               ...
               <button className="flex items-center space-x-3 text-[10px] font-bold uppercase tracking-widest text-white hover:text-[#F1F5F9] transition-colors group">

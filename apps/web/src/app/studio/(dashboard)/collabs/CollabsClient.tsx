      ...
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
           ...
           <Link 
             ...
             className="bg-[#F1F5F9] hover:bg-[#2952CC] text-white px-8 py-4 rounded-2xl font-bold transition-all shadow-[0_20px_40px_rgba(51,102,255,0.2)] flex items-center gap-3"
           >
        ...
        <div className="flex items-center gap-2 border-b border-white/5 pb-0">
           {['incoming', 'sent', 'active', 'completed'].map((tab) => (
             ...
             >
               {tab}
               {activeTab === tab && <div className="absolute bottom-0 left-0 w-full h-1 bg-[#F1F5F9]"></div>}
             </button>
           ))}
        </div>
                 ...
                 activeDeals.map((deal: any) => (
                   <div key={deal.id} className="bg-[#0A0A0A] border border-white/5 rounded-[40px] p-8 flex flex-col md:flex-row gap-8 items-center">
                      ...
                      <div className="flex gap-4 w-full md:w-auto">
                        ...
                        <button className="flex-1 md:flex-none px-8 py-4 bg-[#F1F5F9] rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-[#2952CC] transition-all">Message</button>
                      </div>
                   </div>
                 ))
               )}
          ...
          <div className="w-full md:w-64 space-y-6">
             <div className="relative">
                ...
                <div className="absolute -bottom-2 -right-2 bg-black border border-white/10 p-2 rounded-xl">
                   {type === 'incoming' ? <ArrowDownLeft className="w-4 h-4 text-[#F1F5F9]" /> : <ArrowUpRight className="w-4 h-4 text-orange-500" />}
                </div>
             </div>
             <div>
                ...
                <div className="flex items-center gap-2 text-[9px] font-bold text-gray-500 uppercase tracking-widest mt-1">
                   ...
                   <span>•</span>
                   <span className="text-[#F1F5F9]">{otherArtist.supporterCount} SUPPORTERs</span>
                </div>
             </div>
             <div className="flex items-center gap-3">
                {otherArtist.isVerified && <span className="p-1 bg-[#F1F5F9]/10 text-[#F1F5F9] rounded-md"><Check className="w-3 h-3" /></span>}
          ...
          <div className="flex-1 space-y-8">
             <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                <div className="space-y-1">
                   <span className="text-[10px] font-bold text-[#F1F5F9] uppercase tracking-widest">{request.collabType.replace('_', ' ')}</span>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-6 bg-black/40 rounded-3xl border border-white/5 space-y-2">
                   ...
                   <div className="flex items-center gap-2">
                      <Percent className="w-4 h-4 text-[#F1F5F9]" />
             ...
             {request.demoUrl && (
               <Link 
                 ...
                 target="_blank"
                 className="flex items-center gap-3 text-[10px] font-bold text-[#F1F5F9] uppercase tracking-widest hover:text-white transition-colors group"
               >
                  <div className="w-8 h-8 rounded-full bg-[#F1F5F9]/10 flex items-center justify-center group-hover:bg-[#F1F5F9] group-hover:text-white transition-all">
             ...
             {type === 'incoming' && (
               <div className="flex flex-col md:flex-row gap-4 pt-4 border-t border-white/5">
                  <button 
                    onClick={onAccept}
                    className="flex-1 bg-[#F1F5F9] hover:bg-[#2952CC] text-white py-4 rounded-2xl font-bold transition-all flex items-center justify-center gap-2"
                  >

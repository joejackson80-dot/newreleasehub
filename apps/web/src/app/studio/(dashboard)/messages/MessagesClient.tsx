           ...
           <div className="p-8 border-b border-white/5 space-y-6">
              ...
              <div className="relative">
                 ...
                 <input 
                   ...
                   className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-3 text-xs font-bold uppercase tracking-widest text-white focus:outline-none focus:border-[#F1F5F9]"
                 />
              </div>
           </div>
                 ...
                 <div className="divide-y divide-white/5">
                    {conversations.map((conv: any) => (
                       <button 
                         ...
                         className={`w-full p-8 text-left transition-all flex items-start gap-5 group relative ${selectedConversation?.user.id === conv.user.id ? 'bg-[#F1F5F9]/5 border-l-2 border-[#F1F5F9]' : 'hover:bg-white/5 border-l-2 border-transparent'}`}
                       >
                          ...
                          <div className="flex-1 min-w-0">
                             <div className="flex items-center justify-between mb-1">
                                <h4 className="font-bold text-white truncate group-hover:text-[#F1F5F9] transition-colors uppercase tracking-tight">{conv.user.displayName || conv.user.username}</h4>
           ...
           {selectedConversation ? (
                    ...
                    <div className="flex items-center gap-5">
                       ...
                       <div>
                          ...
                          <div className="flex items-center gap-2">
                             <ShieldCheck className="w-3 h-3 text-[#F1F5F9]" />
                       ...
                       return (
                          <div key={m.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                             <div className={`max-w-[70%] space-y-2 ${isMine ? 'items-end' : 'items-start'} flex flex-col`}>
                                <div className={`p-6 rounded-3xl text-sm font-medium leading-relaxed ${isMine ? 'bg-[#F1F5F9] text-white rounded-tr-none shadow-[0_0_20px_rgba(241,245,249,0.3)]' : 'bg-[#111] border border-white/5 text-gray-300 rounded-tl-none'}`}>
                                   {m.text}
                                </div>
                 ...
                 <div className="p-8 border-t border-white/5 bg-[#050505]">
                    ...
                    >
                       <input 
                         ...
                         className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-8 py-5 text-sm text-white focus:outline-none focus:border-[#F1F5F9]/50 transition-all shadow-inner disabled:opacity-50"
                       />
                       <button 
                         disabled={isSending || !replyText.trim()}
                         className="w-16 h-16 rounded-2xl bg-white text-black flex items-center justify-center hover:bg-[#F1F5F9] hover:text-white transition-all hover:scale-105 active:scale-95 shadow-2xl disabled:opacity-50 disabled:scale-100"
                       >
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-20 text-center space-y-10">
                 <div className="w-32 h-32 rounded-[3rem] bg-white/5 flex items-center justify-center relative">
                    <div className="absolute inset-0 bg-[#F1F5F9]/20 blur-3xl opacity-50 animate-pulse"></div>

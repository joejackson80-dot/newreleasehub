  ...
  return (
    <>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center p-4 bg-white/5 rounded-2xl border border-white/10">
                ...
                <div>
                  <h4 className="text-[#F1F5F9] font-bold text-lg leading-tight">{currentTrack.title}</h4>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[1000] bg-zinc-950/90 backdrop-blur-2xl border-t border-white/10 py-3 px-4 md:px-6 flex items-center justify-between shadow-[0_-20px_50px_rgba(0,0,0,0.8)]">
         {/* AD OVERLAY (Inline) */}
         {showAd && (
           <div className="absolute inset-0 bg-[#020202] z-50 flex items-center justify-between px-10 animate-in fade-in duration-500 border-t border-[#F1F5F9]/30">
             ...
             <div className="flex flex-col items-center">
               <div className="w-64 h-1.5 bg-white/5 rounded-full overflow-hidden mb-2">
                 <div className="h-full bg-[#F1F5F9] transition-all duration-1000" style={{ width: `${(adTimeRemaining / 30) * 100}%` }}></div>
               </div>
             ...
             <div className="flex items-center space-x-6">
               <Link href="/subscribe" className="text-[#F1F5F9] font-bold text-[10px] uppercase tracking-widest hover:underline">
         ...
         <div className="flex items-center space-x-3 md:space-x-4 w-1/2 md:w-1/3 lg:w-1/4 md:min-w-[250px]">
            ...
            <div className="flex items-center space-x-2 md:space-x-4 overflow-hidden w-full">
               ...
               <button 
                 onClick={toggleLike}
                 className={`hidden sm:block transition-all shrink-0 ${isLiked ? 'text-[#F1F5F9] scale-110' : 'text-gray-400 hover:text-white'} ${likeAnimate ? 'scale-125' : ''}`} 
               ...
               {currentTrack.artistId !== 'nrh-official' && (
                 <Link 
                   ...
                   className="hidden lg:flex items-center space-x-1 px-3 py-1 bg-[#F1F5F9]/10 border border-[#F1F5F9]/30 rounded-full text-[#F1F5F9] hover:bg-[#F1F5F9]/20 transition-all shrink-0"
                 >
         ...
         <div className="flex flex-col items-center justify-center flex-1 md:w-1/3 lg:w-2/4 max-w-[600px]">
            <div className="flex items-center justify-end md:justify-center space-x-4 md:space-x-6 mb-0 md:mb-2 w-full md:w-auto">
               <button onClick={toggleShuffle} className={`hidden md:block p-2 transition-colors ${isShuffle ? 'text-[#F1F5F9]' : 'text-gray-400 hover:text-white'}`} title="Shuffle">
               ...
               <button onClick={toggleRepeat} className={`hidden md:block p-2 transition-colors ${repeatMode !== 'off' ? 'text-[#F1F5F9]' : 'text-gray-400 hover:text-white'}`} title="Repeat">
            ...
            <div className="hidden md:flex items-center w-full space-x-3 text-xs font-medium text-gray-400">
               ...
               >
                  <div className="h-full bg-white group-hover:bg-[#F1F5F9] relative transition-all duration-100 ease-linear" style={{ width: `${(progress / duration) * 100}%` }}></div>
               </div>
         ...
         <div className="hidden md:flex items-center justify-end space-x-3 w-1/3 lg:w-1/4 min-w-[200px] text-gray-400">
            ...
            <button 
              ...
              className={`p-2 transition-colors ${showQueue ? 'text-[#F1F5F9]' : 'hover:text-white'}`}
              title="Queue"
            >
            ...
            <div className="hidden md:flex items-center space-x-2 group ml-2">
               ...
               >
                  <div className="h-full bg-gray-300 group-hover:bg-[#F1F5F9] transition-all" style={{ width: `${volume * 100}%` }}></div>
               </div>
            </div>

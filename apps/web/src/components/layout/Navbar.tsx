            ...
            {NAV_ITEMS.map((item) => {
               ...
               return (
                 <Link 
                   key={item.label} 
                   href={item.href}
                   className={`text-[10px] font-bold uppercase tracking-widest transition-all ${isActive ? 'text-[#F1F5F9]' : 'text-gray-400 hover:text-white'}`}
                 >
                   {item.label}
                 </Link>
               );
        ...
        <div className="hidden lg:flex items-center space-x-6">
           <form action="/search" method="GET" className="relative group">
              <input 
                ...
                name="q"
                placeholder="Search..." 
                className="bg-white/5 border border-white/10 rounded-full px-6 py-2.5 text-[10px] font-bold text-white placeholder-gray-700 focus:outline-none focus:border-[#F1F5F9] transition-all w-48"
              />
              <Search className="absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-700" />
           </form>
           <div className="flex items-center space-x-4">
              <Link href="/login" className="text-[10px] font-bold uppercase tracking-widest text-white hover:text-[#F1F5F9] transition-colors">Sign In</Link>
        ...
        {isOpen && (
          ...
          >
             ...
             <div className="p-8 border-t border-white/5 bg-[#0A0A0A] space-y-4">
                ...
                <Link href="/register" onClick={() => setIsOpen(false)} className="w-full py-5 rounded-2xl bg-[#F1F5F9] text-white font-black text-[11px] uppercase tracking-[0.2em] text-center block shadow-[0_10px_30px_rgba(241,245,249,0.3)]">Join Free</Link>
             </div>
          </motion.div>
        )}
      </AnimatePresence>

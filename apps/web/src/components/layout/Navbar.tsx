            ...
            {NAV_ITEMS.map((item) => {
               ...
               return (
                 <Link 
                   key={item.label} 
                   href={item.href}
                   className={`text-[10px] font-bold uppercase tracking-widest transition-all ${isActive ? 'text-[#A855F7]' : 'text-gray-400 hover:text-white'}`}
                 >
                   {item.label}
                 </Link>
               );
        ...
        <div className="hidden lg:flex items-center space-x-6">
           <form action="/search" method="GET" className="relative group">
              <input 
                ...
                name="q"
                placeholder="Search..." 
                className="bg-white/5 border border-white/10 rounded-full px-6 py-2.5 text-[10px] font-bold text-white placeholder-gray-700 focus:outline-none focus:border-[#A855F7] transition-all w-48"
              />

             ...
             <div className="space-y-4">
                {items.filter(i => i.type === 'artist').map((artist, idx) => (
                   <Link 
                     key={artist.id}
                     href={`/${artist.slug}`}
                     className="flex items-center gap-4 p-4 bg-[#0A0A0A] border border-white/5 rounded-2xl hover:border-[#F1F5F9]/30 transition-all group"
                   >
                      ...
                      <div className="text-[#F1F5F9] opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                {items.filter(i => i.type === 'opportunity').map((opp) => (
                   <div key={opp.id} className="bg-[#111] border border-white/5 rounded-[2.5rem] p-8 space-y-6 hover:border-white/10 transition-all relative overflow-hidden group">
                      ...
                      <div className="flex justify-between items-start">
                         <span className="px-2 py-1 rounded bg-[#F1F5F91a] text-[#F1F5F9] text-[8px] font-black uppercase tracking-widest border border-[#F1F5F91a]">{opp.category}</span>
                      ...
                      <div className="pt-6 border-t border-white/5 flex items-center justify-between">
                         ...
                         <Link href="/network/board" className="text-[9px] font-bold text-white uppercase tracking-widest hover:text-[#F1F5F9] transition-colors">View Initiative →</Link>
                      </div>
                   </div>
                ))}
             </div>
